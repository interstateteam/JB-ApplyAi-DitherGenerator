...
export const initThreeGrid = (imgWidth, imgHeight, settings) => {
  ...
  // Only tear down and rebuild the InstancedMesh/geometry when something
  // that actually affects mesh *capacity or shape* has changed. Most slider
  // drags (gravity, white cutoff, scale, lightness curve, distortion) leave
  // cols/rows/shapeType producing the same maxInstances, so we can reuse the
  // existing mesh and just let applyImageToGrid recompute matrices/colors
  // on it — skipping a full dispose + GPU buffer allocation every redraw.
  const needsRebuild =
    !instancedMesh ||
    shapeType !== currentShapeType ||
    maxInstances !== currentMaxInstances;

  if (needsRebuild) {
    cleanup();
    currentGeometry = createWarpedGeometry(chaosLevel, shapeType);
    instancedMesh = new THREE.InstancedMesh(currentGeometry, material, maxInstances);
    scene.add(instancedMesh);
    currentShapeType = shapeType;
    currentMaxInstances = maxInstances;
  }
...
export const applyImageToGrid = (imgData, cols, rows, settings, mesh) => {
  ...
  // Hoisted out of the per-cell gravity loop below — this used to be a live
  // document.getElementById() DOM query running once for every subject
  // cell (thousands of times per redraw), even though its result never
  // changes within a single redraw.
  ...
  // getBrightnessRange() sorts every visible pixel — only needs to re-run
  // when the sampled image data has actually changed. Keyed on object
  // identity of imgData rather than (cols, rows) alone, so this stays
  // correct for callers that pass a fresh imgData every call (gif
  // playback, video export), not just the cached path in updateThreeGrid.
  let minBright, maxBright;
  if (imgData) {
    if (brightnessRangeCache && brightnessRangeCache.imgData === imgData) {
      ({ minBright, maxBright } = brightnessRangeCache);
    } else {
      ({ minBright, maxBright } = getBrightnessRange(imgData));
      brightnessRangeCache = { imgData, minBright, maxBright };
    }
  }
  ...
  // Structure-of-arrays instead of an array of { col, row, brightness, ... }
  // objects — the old version allocated one small object per subject cell,
  // every redraw (easily thousands), which meant real GC pressure while
  // dragging sliders. Typed arrays sized to the worst case (every cell is
  // subject) avoid that churn entirely; subjectCellCount tracks how many
  // are actually filled in.
  const scCol = new Int32Array(count);
  const scRow = new Int32Array(count);
  const scBrightness = new Float32Array(count);
  const scDarkness = new Float32Array(count);
  const scShiftX = new Float32Array(count);
  const scShiftY = new Float32Array(count);
  const scDarkPullX = new Float32Array(count);
  const scDarkPullY = new Float32Array(count);
  const scEffectiveEdge = new Float32Array(count);
  const scIsBoundary = new Uint8Array(count);
  const scIsInternalEdge = new Uint8Array(count);
  let subjectCellCount = 0;
  ...
  for (let col = 0; col < cols; col++) {
    for (let row = 0; row < rows; row++) {
      ...
      ({ shiftX, shiftY, edgeProximity } = calculateGravityShift(
        col, row, cols, rows, brightnessCache, alphaCache,
        alphaCache[cacheIndex], smallnessInfluence, pixelGravity, spacing, alignmentFactor,
      ));
      ...
      const i = subjectCellCount++;
      scCol[i] = col;
      scRow[i] = row;
      scBrightness[i] = brightness;
      scDarkness[i] = darkness;
      scShiftX[i] = shiftX;
      scShiftY[i] = shiftY;
      scDarkPullX[i] = darkPullX;
      scDarkPullY[i] = darkPullY;
      scEffectiveEdge[i] = effectiveEdge;
      scIsBoundary[i] = isBoundary ? 1 : 0;
      scIsInternalEdge[i] = isInternalEdge ? 1 : 0;
    }
  }
  ...
  for (let i = 0; i < subjectCellCount && instanceIndex < maxInstances; i++) {
    const cellCol = scCol[i];
    const cellRow = scRow[i];
    const cellBrightness = scBrightness[i];
    const cellDarkness = scDarkness[i];
    const cellShiftX = scShiftX[i];
    const cellShiftY = scShiftY[i];
    const cellDarkPullX = scDarkPullX[i];
    const cellDarkPullY = scDarkPullY[i];
    const cellEffectiveEdge = scEffectiveEdge[i];
    const cellIsBoundary = scIsBoundary[i] === 1;
    const cellIsInternalEdge = scIsInternalEdge[i] === 1;
    ...
    const isCrispEdgeCell = cellIsBoundary || cellEffectiveEdge > 0.6 || cellIsInternalEdge;

    const edgeGuaranteeProbability = THREE.MathUtils.smoothstep(cellDarkness, 0.0, 0.25);
    ...
    for (let d = 0; d < numDots && instanceIndex < maxInstances; d++) {
      ...
      const baseScatter = isCrispEdgeCell
        ? spacing * (cellIsInternalEdge && !cellIsBoundary
            ? 0.08
      ...
      const pullWeight = THREE.MathUtils.smoothstep(cellDarkness, 0.05, 0.80);
      const totalShiftX = cellShiftX + (cellDarkPullX * pullWeight) + stippleJitterX;
      const totalShiftY = cellShiftY + (cellDarkPullY * pullWeight) + stippleJitterY;
      ...
      const normalizedNoise = hashNoise(cellCol, cellRow);
      const spatialNoise = normalizedNoise * 2 - 1;

      const outlineEase = THREE.MathUtils.smoothstep(cellEffectiveEdge, 0.10, 1.0);
      const outlineBoost = 0.10 * Math.pow(outlineEase, 2.2) * THREE.MathUtils.smoothstep(cellDarkness, 0.0, 0.3);
      const internalEdgeBoost = cellIsInternalEdge ? 0.04 : 0;

      let finalDotSize = innerSize + Math.max(outlineBoost, internalEdgeBoost);

      const midToneWeight = Math.sin(THREE.MathUtils.clamp(cellDarkness, 0.0, 1.0) * Math.PI);
      ...
      const opticalShrink = THREE.MathUtils.lerp(
        ...
        THREE.MathUtils.smoothstep(cellDarkness, 0.0, 0.2)
      );
      ...
      // --- Z-DEPTH CALCULATION ---
      const toneDiff = cellBrightness - midpoint;
      let baseZ = 0;
      ...
      objPos.set(
        (cellCol - (cols - 1) / 2) * spacing + totalShiftX,
        (cellRow - (rows - 1) / 2) * spacing + totalShiftY,
        finalZ,
      );
      ...
      const depthStep = ((cellCol + cellRow) % 8) - 4;
      const gPos = gridPositions[instanceIndex];
      gPos.set(
        (cellCol - (cols - 1) / 2) * spacing * spreadX,
        (cellRow - (rows - 1) / 2) * spacing * spreadY,
        depthStep * spacing * spreadZ,
      );

      const finalGridScale = (pixelScale / 100) * (gridScale / 5) * 0.6;

      const instanceQuat = originalRotations[instanceIndex];
      instanceQuat.setFromEuler(eulerScratch);
      ...
      const dotGrey = THREE.MathUtils.lerp(0.08, 0.92, cellBrightness);
      colorHelper.setScalar(dotGrey);
      mesh.setColorAt(instanceIndex, colorHelper);

      // Compose the matrix directly instead of routing through
      // dummyObject.position/rotation/scale + updateMatrix(). That path
      // sets dummyObject.rotation from eulerScratch, which internally
      // re-derives a quaternion from the euler angles via a change
      // listener — a second euler->quaternion conversion, on top of the
      // one we already did for originalRotations above. Reusing that
      // quaternion here skips the duplicate work and the Object3D
      // bookkeeping entirely.
      scaleScratch.setScalar(finalOriginalScale);
      matrixScratch.compose(objPos, instanceQuat, scaleScratch);
...
export const updateThreeGrid = (img, settings) => {
  ...
  // sampleImage() does a drawImage + getImageData — a GPU->CPU readback,
  // one of the more expensive things to do per frame. Most sliders
  // (gravity, whiteCutoff, pixelScale, gridScale, lightnessCurve) don't
  // change the source image or resolution at all, so re-sampling on every
  // one of those redraws was pure waste. Only pixelAmount (which changes
  // cols/rows) or a new source image actually needs a fresh sample.
  let imgData;
  if (
    sampledImgCache &&
    sampledImgCache.img === img &&
    sampledImgCache.cols === cols &&
    sampledImgCache.rows === rows
  ) {
    imgData = sampledImgCache.imgData;
  } else {
    imgData = sampleImage(img, cols, rows);
    sampledImgCache = { img, cols, rows, imgData };
  }
